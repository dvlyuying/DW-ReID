...
def make_optimizer_2stage(cfg, model, center_criterion):
    ...
    for key, value in model.named_parameters():
        ...
        if "net" in key:
            params_weather+= [{"params": [value], "lr": lr*100, "weight_decay": weight_decay}]
            keys += [key]
        else:
            params += [{"params": [value], "lr": lr, "weight_decay": weight_decay}]
            keys += [key]
    ...
    optimizer_weather = torch.optim.AdamW(params_weather, lr=cfg.SOLVER.STAGE2.BASE_LR*100, weight_decay=cfg.SOLVER.STAGE2.WEIGHT_DECAY)


    return optimizer, optimizer_center,optimizer_weather
